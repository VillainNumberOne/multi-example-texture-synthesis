...
from mexts.function import image_to_tensor, square_crop
...
class OpenImageDialog(tk.Toplevel):
    ...
    def show(self):
        ...
        self.crop_checkbox_var.set(1)
        self.crop_checkbox = tk.Checkbutton(self.frame, text='Square crop', variable=self.crop_checkbox_var)
        self.crop_checkbox.grid(row=1, column=0, sticky='W', padx=5)

        self.resize_w_label = tk.Label(self.frame, text="Width:")
        self.resize_w_label.grid(row=2, column=0, sticky='W')
        self.resize_w_entry = tk.Entry(self.frame, width=5)
        self.resize_w_entry.insert(tk.END, 256)
        self.resize_w_entry.grid(row=2, column=1, sticky='W', padx=5)

        self.resize_h_label = tk.Label(self.frame, text="Height:")
        self.resize_h_label.grid(row=3, column=0, sticky='W')
        self.resize_h_entry = tk.Entry(self.frame, width=5)
        self.resize_h_entry.insert(tk.END, 256)
        self.resize_h_entry.grid(row=3, column=1, sticky='W', padx=5)


        self.scale_label = tk.Label(self.frame, text="Scale (K):")
        self.scale_label.grid(row=4, column=0, sticky='W')
        self.scale_entry = tk.Entry(self.frame, width=5)
        self.scale_entry.insert(tk.END, 2)
